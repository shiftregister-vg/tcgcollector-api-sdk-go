...
func TestListCardReferences(t *testing.T) {
	...
	t.Run("success with pagination", func(t *testing.T) {
		...
		page := 2
		pageSize := 50
		result, err := client.ListCardReferences(context.Background(), &ListCardReferencesParams{
			Page:     &page,
			PageSize: &pageSize,
		})
		assert.NoError(t, err)
...
func TestGetCardReference(t *testing.T) {
	...
	t.Run("not found", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			json.NewEncoder(w).Encode(ErrorResponse{
				Message: "Card reference not found",
				Code:    "NOT_FOUND",
			})
		}))
		defer server.Close()
		...
		client := NewClient("test-api-key", WithBaseURL(server.URL))
		result, err := client.GetCardReference(context.Background(), 999)
		assert.Error(t, err)
		assert.Nil(t, result)
		assert.Contains(t, err.Error(), "Card reference not found")
	})
